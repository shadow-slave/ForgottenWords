...
function createWordCards() {
  ...
  let storedData = JSON.parse(localStorage.getItem("wordData")) || {
    ...
    todaysWords: [],
    flippedWords: [],
    flippedDates: {}, // Ensure flippedDates is initialized
  };

  if (storedData.date !== getTodayDateString()) {
    storedData.date = getTodayDateString();
    storedData.todaysWords = [];
    storedData.flippedWords = [];
    storedData.flippedDates = {}; // Reset flipped dates for a new day
  }
  ...
  storedData.todaysWords.forEach((wordData) => {
    ...
    card.addEventListener("click", () => {
      // console.log("Clicked:", wordData.word);

      const today = getTodayDateString();
      if (!storedData.flippedDates) storedData.flippedDates = {}; // Ensure it exists

      if (storedData.flippedDates[wordData.word] === today) {
        document.getElementById("error").innerHTML="Already flipped today:" + wordData.word;
        return;
      }
...
function displayViewedWords() {
  ...
  // added safety check
  if (!viewedSection) {
    console.error("Element with id 'viewed-words' not found.");
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  createWordCards();
  displayViewedWords(); //display data after the dom is ready
});
// debugger;
